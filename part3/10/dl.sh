    ...
    if [ "$2" != "None" ] && [ -f "$outputpath" ]; then
        ...
        if [ $target_size != "0" ]; then
            ...
            if ((video_size_bits <= target_size)); then
                ...
                if [ "$output_size" -gt "$input_size" ] || [ "$output_size" -eq 0 ]; then
                    echo "Conversion failed to reach the target file size. Keeping the original file."
                    # rm output file
                    rm -f $output_file
                else
                    echo "Conversion successful. Deleting the original file."
                    # rm original
                    rm -f $input_file
                    # rename to original
